...
CekKkSkill checks KK information by dialing USSD code *185#.
...
class TestCekKkSkillSuccess(unittest.TestCase):
    """Test CekKkSkill success paths."""

    def test_success_with_payload(self) -> None:
        """USSD dial returns non-empty text → skill succeeds."""
        mock_runtime = MagicMock()
        mock_runtime.dial.return_value = "Data KK: 3201xxxx"
        skill = CekKkSkill(ussd_runtime=mock_runtime)
    ...
    def test_returns_raw_response(self) -> None:
        ...
        skill = CekKkSkill(ussd_runtime=mock_runtime)
        result = skill.execute("COM3")
        self.assertEqual(result.data["raw_response"], "Data KK: 3201xxxx")
...
class TestCekKkSkillFailure(unittest.TestCase):
    ...
    def test_failure_when_no_ussd_runtime(self) -> None:
        ...
        skill = CekKkSkill(ussd_runtime=None)
        result = skill.execute("COM3")
        self.assertFalse(result.success)
...
class TestCekKkSkillTimeout(unittest.TestCase):
    """Test CekKkSkill timeout handling."""

    def test_custom_timeout_passed(self) -> None:
        """Custom timeout kwarg should be forwarded to dial."""
        mock_runtime = MagicMock()
        mock_runtime.dial.return_value = "KK info"
        skill = CekKkSkill(ussd_runtime=mock_runtime)
...
class TestCekKkSkillName(unittest.TestCase):
    """Test CekKkSkill identity."""

    def test_skill_name_is_cek_kk(self) -> None:
        """name property should return 'cek_kk'."""
        mock_runtime = MagicMock()
        skill = CekKkSkill(ussd_runtime=mock_runtime)
