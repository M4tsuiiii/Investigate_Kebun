...
def init_database():
    try:
        ...
        for nomor, nik, kk, timestamp in cursor.fetchall():
            phone_cache[nomor] = {
                "nik": nik,
                "kk": kk,
...
def classify_ussd_evidence(payload):
    ...
    return {
        "text": text,
        "has_success_evidence": any(k in low for k in ("sukses", "berhasil", "success", "completed")),
        "has_processing_evidence": any(k in low for k in ("sedang diproses", "sedang di proses", "di proses", "dalam proses", "permintaan diterima", "diterima", "accepted", "processing", "in process", "queued", "diproses")),
        "has_tenggang_evidence": bool(re.search(r"\b(dalam\s+masa\s+tenggang|masa\s+tenggang|sedang\s+dalam\s+masa\s+tenggang|sedang\s+dalam\s+tenggang)\b", low)),
    }


def classify_ussd_intent(payload):
    """Classify the semantic intent of a USSD payload string.

    Returns one of: NUMBER_INFO, MENU_RESPONSE, REQUEST_ACCEPTED, SUCCESS_MESSAGE, EMPTY_RESPONSE, UNKNOWN
    """
    ...
    low = text.lower()
    # NUMBER_INFO: contains an Indonesian mobile number pattern
    if re.search(r"08\d{9,11}", low):
        return "NUMBER_INFO"
    # SUCCESS_MESSAGE
    if evidence["has_success_evidence"]:
        return "SUCCESS_MESSAGE"
    # REQUEST_ACCEPTED (processing/accepted)
    if evidence["has_processing_evidence"]:
        return "REQUEST_ACCEPTED"
    # MENU_RESPONSE: greetings, try again, menu listings or short non-actionable prompts
    menu_keywords = ("try again", "good evening", "welcome", "menu", "account", "content", "impoint", "your number", "pilih", "silakan pilih", "silahkan pilih")
    if any(k in low for k in menu_keywords) or re.search(r"(^|\n)\s*\d+\.", payload):
...
class PortWorker(threading.Thread):
    ...
    def log(self, text):
        ...
        msg = f"[{ts}] {text}"
        if self.port_name not in port_logs: port_logs[self.port_name] = []
        port_logs[self.port_name].append(msg)
        if len(port_logs[self.port_name]) > 100: port_logs[self.port_name].pop(0)
                ...
                while self.is_running and self.port_name not in disabled_ports:
                    ...
                    if old_state == "READY" and detected_state == "UNKNOWN":
                        ...
                        if self.unknown_failure_count >= CPIN_UNKNOWN_THRESHOLD:
                            ...
                            self._log_cpin_decision = getattr(self, '_log_cpin_decision', None)
                            try:
                                if hasattr(self, '_log_cpin_decision') and callable(self._log_cpin_decision):
                                    self._log_cpin_decision(old_state, final_raw, final_state, self.unknown_failure_count, decision, reason)
                                else:
                                    # minimal lifecycle log if method not present
    ...
    def send_ussd(self, ser, cmd):
        ...
        if kind == "USSD_EMPTY_PAYLOAD":
            ...
            # Give modem/operator a small window to emit the real payload
            try:
                self._flush_serial_input(ser)
            except Exception:
                pass
    ...
    def execute_full_flow(self, ser):
        ...
        for _ in range(15):
            if not self.is_running or self.port_name in disabled_ports:
                return
            time.sleep(1)
    ...
    def check_log_trigger(self, event):
        try:
            ...
            if item and column == "#8":
                ...
                for l in port_logs.get(item, ["Belum ada data log."]): txt.insert(tk.END, l + "\n")
